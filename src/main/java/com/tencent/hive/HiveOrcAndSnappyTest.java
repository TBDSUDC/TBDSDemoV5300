...
import java.sql.*;
/**
 * hive jdbc 操作orc格式且开启snappy压缩的数据：包含删除表，创建表，insert数据，load数据，查询数据，show table等操作。
 */
public class HiveOrcAndSnappyTest {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            stmt.execute("create table users(user_id int, fname string,lname string )  partitioned by(ds string) stored as orc TBLPROPERTIES ('orc.compression'='SNAPPY')");
            //拥有HDFS文件读写权限的用户才可以进行此操作
            ...
            stmt.execute("load data  inpath '/myhdfs/data.orc' into table users partition(ds='2023-01-13')");//拥有HDFS文件读写权限的用户才可以进行此操作

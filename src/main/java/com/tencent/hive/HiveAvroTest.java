...
/**
 * hive jdbc 操作avro格式的数据：包含删除表，创建表，insert数据，load数据，查询数据，show table等操作。
 *
 */
public class HiveAvroTest {
    static Logger logger = Logger.getLogger(HiveAvroTest.class);
    public static void main(String[] args) {
        ...
        * */
        ...
        try {
            ...
            stmt.execute("create table users(user_id int, fname string,lname string )  row format serde 'org.apache.hadoop.hive.serde2.avro.AvroSerDe' stored as INPUTFORMAT  'org.apache.hadoop.hive.ql.io.avro.AvroContainerInputFormat' OUTPUTFORMAT 'org.apache.hadoop.hive.ql.io.avro.AvroContainerOutputFormat' TBLPROPERTIES ('avro.schema.url'='/myhdfs/user.avsc')");
            //拥有HDFS文件读写权限的用户才可以进行此操作
            ...
            stmt.execute("load data  inpath '/myhdfs/avrodata.avro' into table users");//拥有HDFS文件读写权限的用户才可以进行此操作

...
import java.sql.*;
/**
 * hive jdbc 操作parquet格式且开启snappy压缩的数据：包含删除表，创建表，insert数据，load数据，查询数据，show table等操作。
 */
public class HiveParquetTest {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            stmt.execute("create table users(user_id int, fname string,lname string )  STORED AS PARQUET TBLPROPERTIES('parquet.compression'='SNAPPY')");
            //拥有HDFS文件读写权限的用户才可以进行此操作
            ...
            stmt.execute("load data  inpath '/myhdfs/data.parquet' into table users");//拥有HDFS文件读写权限的用户才可以进行此操作

...
/**
 * hive jdbc 操作json格式的数据：包含删除表，创建表，insert数据，load数据，查询数据，show table等操作。
 */
public class HiveJsonTest {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            stmt.execute("create table users(user_id int, fname string,lname string )  row format serde 'org.apache.hive.hcatalog.data.JsonSerDe' stored as textfile");
            //拥有HDFS文件读写权限的用户才可以进行此操作
            ...
            stmt.execute("load data  inpath '/myhdfs/data.json' into table users");//拥有HDFS文件读写权限的用户才可以进行此操作
